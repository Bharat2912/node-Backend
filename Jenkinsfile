 ...
 stage("Code Checkout from GitHub") {
  steps {
      script {
               try{        
          sh "rm -rf *"
          git branch: "main" , credentialsId: 'creds' ,url: 'https://github.com/Bharat2912/node-Backend.git'
   }
        catch(Exception e) {
          echo "FAILED ${e}"
          currentBuild.result = 'FAILURE'
      ...
      stage ('OWASP Dependency-Check Vulnerabilities') {
        ...
        stage('Deploying to Dev Enviroment') {
            steps {
                ...
                docker push 847280823661.dkr.ecr.us-east-1.amazonaws.com/backend:latest
                docker push 847280823661.dkr.ecr.us-east-1.amazonaws.com/backend-rollback:$Commit
                              
                aws ecs update-service --cluster scrum-backend-cluster --service backend --region us-east-1 --force-new-deployment 
                sleep 15
